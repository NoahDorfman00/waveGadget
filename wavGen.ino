...
void aISR() {
  ...
  if (now - lastA >= 500) {
    if (++freqIdx >= NUM_FREQS)
      freqIdx = 0;
    Serial.println(freqs[freqIdx]);
    // TC0->COUNT16.CTRLA.bit.ENABLE = 0;                                      // Enable the TC0 timer
    // while (TC0->COUNT16.SYNCBUSY.bit.ENABLE);

    TC0->COUNT16.CC[0].reg = CYCLES(freqs[freqIdx]);         // Set the sine wave frequency to 1kHz: 48MHz / (1000 * 1000) - 1
